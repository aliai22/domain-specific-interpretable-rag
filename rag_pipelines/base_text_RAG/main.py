...
def run_rag_pipeline(query=None):
    ...
    dataset, metadata = dataset_loader.load_dataset(path = dataset_path)
    proc_dataset = data_processor.preprocess_dataset(data=dataset, metadata=metadata)
   ...
   # Initialize vector store
    ...
    # Initialize chatbot
    chatbot = RAGChatbot(model=base_model, tokenizer=eval_tokenizer)
